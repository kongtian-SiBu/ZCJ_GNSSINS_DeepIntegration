function [trackResults, channel]= trackpll3rd2(fid, channel, settings)
...
tau1carr = b3 * wn / settings.pllLoopGain;
tau2carr = a3 * wn^2 / settings.pllLoopGain * PDIcarr / 2;
tau3carr = wn^3 / settings.pllLoopGain * PDIcarr^2 / 4;
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        % 载波环环路滤波器的中间量初始化
        oldoldCarEor = 0.0;          % 都是环路滤波过程中的状态量
        oldCarrError = 0.0;
        oldCarrNco = 0.0;
        oldoldCarNco = 0.0;
...
%% 处理一个相干积分数据块 ------------------------------------------------                        
            ...
            rawSignal = awgn(rawSignal, -20, 'measured') ./ 10;   % 添加噪声， ./10是为了防止数值过大，无其他意义
...
%% PLL环路更新 -----------------------------------------------------
            ...
            % 环路滤波器的时域公式，是从Z域转换得到
            carrNco = (2 * oldCarrNco - oldoldCarNco) + tau1carr * (carrError - 2 * oldCarrError + oldoldCarEor) + ...
                      tau2carr * (carrError - oldoldCarEor) + tau3carr * (carrError + 2 * oldCarrError + oldoldCarEor);
            oldoldCarNco = oldCarrNco;    oldoldCarEor = oldCarrError;
            oldCarrNco   = carrNco;       oldCarrError = carrError;
