function [trackResults, channel]= trackfll1stpll2nd(fid, channel, settings, isFLL)
...
%% PLL环路更新 -----------------------------------------------------
            if loopCnt == 1
                ...
                if isFLL == 1
                    carrError = 0;  % 去除此行，为一阶FLL辅助二阶PLL；反之则为纯一阶FLL
                end
                % 环路滤波器的时域公式，是从Z域转换得到
                % 完全模仿gnss_sdrlib这份C语言代码
                carrNco = oldCarrNco + (tau2carr/tau1carr) * ...
                    (carrError - oldCarrError) + carrError * (PDIcarr/tau1carr) + freqErr /(2*pi) * PDIcarr * wnf; % 角频率/2pi等于频率
                oldCarrNco   = carrNco;
