function [trackResults, channel]= trackfll2ndpll3rd(fid, channel, settings, isFLL)
...
%       isFLL           - isFLL = 1, 则断开PLL变为纯FLL；否则为FLL辅助PLL
%   Outputs:
...
a3 = 1.1;
b3 = 2.4;
wn = settings.pllNoiseBandwidth / 0.7845;           % 注意，settings.pllLoopGain是在分母上
kca1 = wn^3 * PDIcarr / settings.pllLoopGain;
kca2 = a3 * wn^2 * PDIcarr / settings.pllLoopGain;
kca3 = b3 * wn / settings.pllLoopGain;

%--- FLL 参数 --------------------------------------------------------
% 2阶FLL环路滤波器系数计算
a2 = 1.414;
wn = settings.fllNoiseBandwidth / 0.53;
fca1 = wn ^ 2 * PDIcarr / settings.pllLoopGain;
fca2 = a2 * wn * PDIcarr / settings.pllLoopGain;
...
%% PLL环路更新 -----------------------------------------------------
            if loopCnt == 1
                ...
                % fll discriminator, 完全模仿gnss_sdrlib这份C语言代码,并没有完全按照书上所写的鉴频器
                if I_P == 0
                    f1 = pi/2;
                else
                    f1 = atan(Q_P/I_P);    % 当前载波相位
                end

                if trackResults(channelNr).I_P(loopCnt - 1) == 0
                    f2 = pi/2;
                else
                    f2 = atan(trackResults(channelNr).Q_P(loopCnt-1)/trackResults(channelNr).I_P(loopCnt - 1)); % 上一时刻载波相位
                end
                freqErr = (f1 - f2);           % 相位步进量
                if (freqErr > pi/2)
                    freqErr = pi-freqErr;
                ...
                freqErr = freqErr / PDIcarr;   % 频率变化量 = 相位步进量 / 时间变化量
                
                % 鉴相器，atan(Q_P / I_P)的单位为弧度，除以2pi后单位变成 '周'，也等价于Hz
                carrError = atan(Q_P / I_P) / (2.0 * pi);
                if isFLL == 1
                    carrError = 0;    % 去除该行后，为2阶FLL辅助三阶PLL；否则为纯2阶FLL
                end
                % 环路滤波器的时域公式，是从Z域转换得到
                xk = carrError * kca1 + freqErr * fca1;  % 第一个双线性变换器的输入
                yk = ykminus1 + (xk + xkminus1);         % 第一个双线性变换器的输出
                xkminus1 = xk;  ykminus1 = yk;
                
                xkk = yk/2 * PDIcarr + carrError * kca2 + freqErr * fca2;  % 第二个双线性变换器的输入
                ykk = ykkminus1 + (xkk + xkkminus1);     % 第二个双线性变换器的输出
                xkkminus1 = xkk;  ykkminus1 = ykk;
                
                carrNco = ykk/2 + carrError * kca3;
