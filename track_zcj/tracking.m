...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        caCodeCN0 = generateCAcode(1);              % 数据集中不包含PRN1这颗卫星，可用于计算噪声，从而计算CN0
        % 前后各添加一个码片
        caCodeForCN0 = [caCodeCN0(1023) caCodeCN0 caCodeCN0(1)];  % 附加通道用于计算CN0(正确性暂不保证)
        caCode = [caCode(1023) caCode caCode(1)];   % 用于生成EPL支路的码相位，在原有的1023个码片的前后各添加一个码片
...
%% 处理一个相干积分数据块 ------------------------------------------------                        
            ...
            rawSignal = transpose(rawSignal);  % 转置，注意这里不要用'。单引号'号表示共轭转置
